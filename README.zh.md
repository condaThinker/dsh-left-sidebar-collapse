...
> 该插件带一个 `prepare` 构建脚本。以 Git 方式安装时会执行该脚本构建 `lib/`，因此 profile 需要允许：先把 `dsh-left-sidebar-collapse` 加入 profile 的 `allowBuilds`（见 `pnpm install` 文档），再执行 `dsh plugin add`。或者，若要免构建安装，可把构建好的 `lib/` 提交进 Git 并去掉 `prepare`。
