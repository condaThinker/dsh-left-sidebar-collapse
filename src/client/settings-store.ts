...
const INITIAL: AutoCollapseSidebarState = {
  enabled: false,
  fullyCollapse: false,
}
...
export class AutoCollapseSidebarController {
  /** Row snapshot the renderer subscribes to. */
  readonly store: SnapshotStore<AutoCollapseSidebarState> = createSnapshotStore(INITIAL, {
  ...
  constructor() {
    ...
    if (snap.fullyCollapse === undefined) this.store.update(s => { s.fullyCollapse = false })
    if (snap.enabled === undefined) this.store.update(s => { s.enabled = false })
  }
